...
// wysyłki (broadcastEmailToAllUsers) ma już pokrycie przez inne broadcasty.
const sendAiUpdateAnnouncementToAllUsersMock = vi.fn();
vi.mock("../services/admin-broadcast.js", () => ({
  sendAiUpdateAnnouncementToAllUsers: (...args: unknown[]) => sendAiUpdateAnnouncementToAllUsersMock(...args),
}));
...
describe("POST /api/admin/announce-ai-update", () => {
  let server: Server;
  let baseUrl: string;
  ...
  it("nie-admin → 403, serwis nie wywołany", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/admin/announce-ai-update`, { method: "POST" });
    expect(res.status).toBe(403);
    expect(sendAiUpdateAnnouncementToAllUsersMock).not.toHaveBeenCalled();
  });

  it("admin → 200 z podsumowaniem serwisu", async () => {
    authState.userId = "user_the_admin";
    sendAiUpdateAnnouncementToAllUsersMock.mockResolvedValueOnce({ totalUsers: 5, sent: 4, skipped: 1, failed: 0 });
    const res = await fetch(`${baseUrl}/api/admin/announce-ai-update`, { method: "POST" });
