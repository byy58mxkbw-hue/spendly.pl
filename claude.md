...
29. **Kwoty: netto vs brutto** — `invoice_items.total_price` i `unit_price` są zapisywane NETTO; `invoices.total_amount` jest BRUTTO (z KSeF `header.totalGross`). Wszystkie widoki wydatków pokazują BRUTTO: raporty, dashboard, statystyki dostawców, rozbicie kategorii i eksport Excel liczą `total_price * (1 + COALESCE(vat_rate,0)/100)`. Dodając nowe zapytanie o wydatki — NIGDY samo `SUM(total_price)`, bo ekran rozjedzie się z Fakturami (realny incydent 2026-08-03: Raporty 144 536 zł vs Faktury 153 184 zł). Uwaga: ten sam wzorzec bywa pisany builderem Drizzle (`${invoiceItemsTable.totalPrice}`) — szukaj OBU form. Każdy kod ustawiający `total_amount` (import ręczny, usuwanie pozycji) musi zapisywać BRUTTO. WYJĄTEK: food cost, kalkulacja dań i AI CFO zostają na NETTO — VAT jest odliczany, więc nie jest kosztem. Migracja wsteczna: `services/ensure-gross-invoice-totals.ts` (idempotentna, przy starcie).
---
...
- Blog SEO (2026-07-14) — czysto statyczny (bez Reacta): `content/blog/*.md` → generator `artifacts/ksef-monitor/scripts/build-blog.mjs` → `public/blog/<slug>.html` + `public/blog.html` (index!) + regenerowany `public/sitemap.xml`. Build odpala generator przed vite; ręcznie `pnpm --filter @workspace/ksef-monitor build:blog`. 9 artykułów (food cost, prime cost, marża/narzut, KSeF, wycena dań, inwentaryzacja, negocjacje). Każdy: BlogPosting+BreadcrumbList JSON-LD. Index MUSI być `blog.html`, NIE `blog/index.html` (sirv na prodzie nie łapie katalog-indexu → SPA 404). Link „Blog” w nav+stopce jako zwykły `<a href="/blog">` (pełne przeładowanie, NIE wouter).
- Search Console + sitemap (2026-07-15) — usługa typu Domena `spendly.pl`, sitemapa `https://www.spendly.pl/sitemap.xml` zgłoszona (Sukces, 17 URL). Indeksowanie pojedynczych stron przez „Sprawdź URL” (górny pasek), NIE przez pole sitemapy.
- Analityka PostHog (2026-07-15) — MCP PostHog podłączony do Claude Code (mogę tworzyć insighty/lejki). Projekt 223258 EU. Dashboard „Spendly — Ruch i SEO" (id 822525): ruch, najpopularniejsze podstrony, wg województwa, źródła wejść. W apce zakładka admin „Analityka" (`admin-analytics.tsx`) osadza dashboard PostHog przez iframe (`VITE_POSTHOG_DASHBOARD_URL` lub domyślny embed) — żeby pokazać nowy dashboard, trzeba go w PostHog udostępnić („Share externally" → Embed) i podmienić URL. Lejek rejestracji do zrobienia gdy `sign_up` zacznie się zbierać.
- Eksport Excel raportów (2026-07-15) — endpoint `GET /api/reports/products-by-cost-center.xlsx?month=YYYY-MM` (`routes/reports-export.ts`, biblioteka `exceljs`, za `requireUser`). Jeden arkusz, centra kosztów pod sobą (nagłówek w kolorze centrum → produkty → wiersz SUMA), ceny BRUTTO (netto×(1+VAT), bo `invoice_items.total_price` to netto), porównanie średniej ceny do poprzedniego miesiąca; produkt spoza poprz. miesiąca = „nowy". Przycisk „Eksport Excel" w `reports.tsx` (fetch z tokenem Clerk, binarny endpoint poza Orval). exceljs bundluje się do `dist/index.mjs` (sprawdzone — reguła 25). Warianty: bez centrum = ogólny (per centrum); z wybranym centrum = per dostawca; kolumny porównania cen + ilości + SUMA per grupa vs poprzedni okres; ustawienia druku (landscape, fit-to-width, powtarzany nagłówek).
- Okres w raportach — zakres [from,to] (2026-07-16) — cały system raportów przeszedł z „miesiąc" na OKRES `[from,to]` z porównaniem do POPRZEDNIEGO RÓWNEGO okresu (miesiąc→miesiąc, kwartał→kwartał, rok→rok; własny zakres dni → równa długość dni wstecz). Wspólny helper `artifacts/api-server/src/lib/period.ts` (`periodFromQuery` fallback month, `previousPeriod` month-aligned + dniowy, `periodLabel`) + bliźniak na froncie `contexts/period-context.tsx` (`PeriodProvider`/`usePeriod`, presety this-month/3m/6m/rok + custom, persist localStorage). Endpointy `monthly`/`spend-bridge`/`category-spend`/`cost-centers` + Excel przyjmują `from`/`to` (openapi+codegen); pojedynczy miesiąc = identyczne wyniki jak dawniej (zero regresji), tryb „all" zachowany. Selektor okresu (presety + zakres dni `input[type=date]`) w nagłówku Raportów. `category-spend-trend` i `predictive` świadomie POZA selektorem (trend/prognoza — wielomiesięczne). Weryfikacja deployu backendu przez publiczny `GET /api/version` (marker buildu — bump przy istotnych zmianach).
